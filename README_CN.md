...
1. **AI 助手** 通过 MCP 发送工具调用（如 `stata_run_command`）。
...
AI 调用: stata_run_command("sysuse auto, clear")
AI 调用: stata_inspect_data()                    -> 74 obs, 12 variables
AI 调用: stata_run_command("regress price mpg weight foreign, robust")
AI 调用: stata_get_results("e", "N,r2,F")        -> N=74, R²=0.52, F=29.1
AI 调用: stata_run_command("scatter price mpg || lfit price mpg")
AI 调用: stata_export_graph(format="png")        -> [图片]
...
| 工具 | 说明 | 示例 |
|------|------|------|
| `stata_run_command` | 交互式执行短命令 | `stata_run_command(code="regress price mpg weight, robust")` |
| `stata_run_do_file` | 批处理模式运行 `.do` 文件（适合长脚本） | `stata_run_do_file(path="/path/to/analysis.do")` |
| `stata_inspect_data` | 描述当前内存中的数据集 | 返回观测数、变量名、类型、标签 |
| `stata_codebook` | 生成指定变量的 codebook | `stata_codebook(variables="price mpg foreign")` |
| `stata_get_results` | 提取存储的结果（r/e/c 类） | `stata_get_results(result_class="e", keys="N,r2")` |
| `stata_export_graph` | 导出当前图形为 PNG/SVG/PDF | 返回 base64 编码的图片数据 |
| `stata_search_log` | 搜索 Stata 会话日志 | `stata_search_log(query="error", regex=true)` |
| `stata_install_package` | 安装 SSC 或用户编写的包 | `stata_install_package(package="reghdfe")` |
| `stata_cancel_command` | 发送中断信号（SIGINT）取消运行中的命令 | `stata_cancel_command(session_id="default")` |
| `stata_list_sessions` | 列出所有活跃的 Stata 会话 | 返回会话 ID、类型、存活状态 |
| `stata_close_session` | 关闭指定的 Stata 会话 | `stata_close_session(session_id="default")` |
...
```
AI 调用: stata_run_command(code="sysuse auto, clear", session_id="session_A")
AI 调用: stata_run_command(code="sysuse nlsw88, clear", session_id="session_B")
